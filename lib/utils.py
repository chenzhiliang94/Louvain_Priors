...
import json
# read louvain priors from json file into a N x N matrix
def read_priors(N):
    f = open('priors.json')
    ...
    # a dictionary
    data = json.load(f)
    coupling_priors = np.zeros((N, N))
    decoupling_priors = np.zeros((N, N))
    
    for couple in data["coupling_priors"]:
        print("coupling priors: ", couple[0], " ", couple[1])
        coupling_priors[couple[0]][couple[1]] = 1
        coupling_priors[couple[1]][couple[0]] = 1
    for decouple in data["decoupling_priors"]:
        print("decoupling priors: ", decouple[0], " ", decouple[1])
        decoupling_priors[decouple[0]][decouple[1]] = 1
        decoupling_priors[decouple[1]][decouple[0]] = 1

    return coupling_priors, decoupling_priors